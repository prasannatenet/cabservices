        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-white dark:bg-[#161615] shadow-sm rounded-2xl border border-gray-100 dark:border-gray-800/60 overflow-hidden">
                <div class="p-6 border-b border-gray-100 dark:border-gray-800/60 flex justify-between items-center">
                    <h3 class="text-lg font-bold text-gray-900 dark:text-white font-display">Drivers in My Cities</h3>
                    <a href="{{ route('associate.drivers.create') }}" class="px-4 py-2 bg-primary-600 hover:bg-primary-700 text-white text-sm font-semibold rounded-lg transition-colors shadow-sm shadow-primary-500/30">
                        + Add Driver
                    </a>
                </div>

                <div class="p-6 pb-0">
                    <x-admin-filter-bar :action="route('associate.drivers.index')">
                        <div>
                        ...
                        <tbody class="bg-white dark:bg-[#161615] divide-y divide-gray-100 dark:divide-gray-800/60">
                            ...
                            <tr class="hover:bg-gray-50/50 dark:hover:bg-[#0a0a0a]/50 transition-colors">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <a href="{{ route('associate.drivers.show', $driver) }}" class="text-blue-600 hover:text-blue-900 dark:text-blue-400">View</a>
                                    <a href="{{ route('associate.drivers.edit', $driver) }}" class="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 ml-3">Edit</a>
                                    <a href="{{ route('associate.drivers.leaves', $driver) }}" class="text-green-600 hover:text-green-900 dark:text-green-400 ml-3">Leaves</a>
                                    <form action="{{ route('associate.drivers.destroy', $driver) }}" method="POST" class="inline-block" onsubmit="return confirm('Delete this driver?');">
                                        @csrf
                                        @method('DELETE')
                            ...
                            @empty
                            <tr>
                                <td colspan="6" class="px-6 py-8 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400 text-center font-medium">No drivers found in your cities.</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
